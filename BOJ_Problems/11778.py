class Matrix:
    def __init__(self, matrix_array): self.matrix = matrix_array; self.base_matrix = matrix_array
    def __mul__(self, other):
        ...
        return Matrix([[((a*e) % P + (b*g) % P) % P, ((a*f) % P + (b*h) % P) % P], [((c*e) % P + (d*g) % P) % P, ((c*f) % P + (d*h) % P) % P]])
    def __pow__(self, k):
