...
def displayFix(name_image):  #고정멘트 이미지 추가
  global fix
  fix = Image("images/fix/"+name_image+".png")
  image_set(fix)
  return

def displayCheck(name_image): #비고정멘트 이미지 추가
  global check
  check = Image("images/check/"+name_image+".png")
  image_set(check)
  return
...
def displayingame():   # 인게임 바탕화면
  global paper
  background_image = Image("images/image/background.png")
...
def displayHowToPlay():  # 게임 방법을 알려주는 화면
  ...
  paper.clear()
  HowToPlay = Layer()
  HowToPlay_background = Image("images/how_to_back.png")
...
def displayRetry():  # 게임 오버 후 새 게임 시작 여부를 묻는 화면
  global paper
  retry_image = Image("images/image/retry.png")   # Try Again? 이미지
...
def displayOutSituation():  # 예측 숫자 입력 결과가 '아웃'일 때 화면
  global paper
  out_image = Image("images/image/out.png")
  image_set(out_image)
  sleep(1)
...
def displayGameOverByWinning():  # 정답을 맞추어 게임이 끝났을 때 화면
  global paper
  end_image = Image("images/image/end.png")
  image_set(end_image)
  sleep(3)
...
def displayEvaluationResult(): # 스트라이크, 볼, 아웃 화면
  ...
  judge_image = Image("images/image/judge.png")   # 볼 스트라이크 판정 이미지
  image_set(judge_image)

  directory_s = "images/num/num" + str(strikes) + "_s.png"
  directory_b = "images/num/num" + str(balls) + "_b.png"
...
def displayStartSec(): # n초후 시작합니다
  ...
  for i in range(5, 0, -1):
    __clear()
    secNum = Image("images/numbers13pt/num"+str(i)+".png")
...
def view_scores(): # 점수 보기
  __clear()
  paper.clear()
  bg = Image("images/image/background.png")
  bg.moveTo(400, 300)
  paper.add(bg)
  Prize = Layer()
...
def game_start(): # 게임 시작 -> 인게임 -> Retry
  ...
  score = tries
  with open("best_user_score.txt", "a+", encoding="utf-8") as f:
    f.write("%s %d\n" % (userName, score))
