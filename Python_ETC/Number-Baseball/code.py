...
def displayRetry():  # 게임 오버 후 새 게임 시작 여부를 묻는 화면
    global paper
    retry_image = Image(resource_path(
        "images/image/retry.png"))   # Try Again? 이미지
...
def displayEvaluationResult():  # 스트라이크, 볼, 아웃 화면
    ...
    judge_image = Image(resource_path(
        "images/image/judge.png"))   # 볼 스트라이크 판정 이미지
...
def view_scores():  # 점수 보기
    ...
    for i in range(ran):
        if sorted_list[i - 1][1] < sorted_list[i][1]:
            ranking += 1
        data = "%2sth\t%10s\t\t%2d" % (
            ranking, sorted_list[i][0], sorted_list[i][1])
