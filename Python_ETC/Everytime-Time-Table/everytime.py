...
tab_control.add(tab1, text='검색')
tab_control.add(tab2, text='담은과목')
tab_control.add(tab3, text='결과')
tab_control.add(tab4, text='내 시간표')
...
def In_Lecture():    # In버튼 누르면 group에 추가
    ...
    for k in range(10):
        if searched_objs[idx] in selected_groups[k]:
            messagebox.showinfo("중복", "이미 담은 과목입니다.")
            dupl = True
            break
...
def search_subjects():    # Search버튼 누르면 조건에 맞는 강의들을 보여줌
    ...
    if name != "강의명이나 교수명으로 검색하세요":    # 검색어에 무언가라도 적혀있는 경우
        ...
        if yeok == "None" and tong == "None" and gae == "None" and \
            ...
            if yeok != "None" and tong == "None" and gae == "None" and \
                ...
                messagebox.showinfo("다중선택", "옵션을 하나만 골라주세요.")

    elif name == "강의명이나 교수명으로 검색하세요" or name == "":    # 검색어 없는 경우
        if yeok == "None" and tong == "None" and gae == "None" and \
                gicho == "None" and gyojik == "None" and ilban == "None" and cs == "None":
            messagebox.showinfo("필터없음", "강의명 또는 교수명으로 검색하거나 옵션 하나를 골라주세요.")    # 아무것도 안고른경우
    ...
    search_by_name_or_prof.insert(0, "강의명이나 교수명으로 검색하세요")
    
    


def shot():
    global tab4_frame1, captured, ldr, scb2, window
    
    w, h = window.winfo_screenwidth(), window.winfo_screenheight()
    a, b, c, d = 2, 25, 533, 707
    img = ImageGrab.grab((a, b, c, d))
    nowdate = datetime.datetime.now()
    re_now = nowdate.strftime("%Y-%m-%d %H%M%S")
    img.save(resource_path('MyTables_/' + re_now + '.png'))
    messagebox.showinfo("성공", "사진을 저장했습니다")
...
def combinating():
    ...
    if len(all_possible_case) >= 60:
        ...
        msg = str(len(all_possible_case)) + "개의 시간표 중 60개를 생성하였습니다.\n결과 탭에서 확인하세요."
    else:
        random.shuffle(all_possible_case)
        test_lists = all_possible_case
        msg = str(len(test_lists)) + "개의 시간표가 생성되었습니다.\n결과 탭에서 확인하세요."
    ...
    Button(tab3_frame2, text="보기", width=10, bg="skyblue", command=see_combi).grid(row=7, column=7)
    Label(tab3_frame2, text = "\n저장기능은 세로 900px 이상의\n해상도(배율 100%)에 최적화되어 있습니다.").grid(row=8, column=7)

    # 조합 콤보박스 위에 뭔지 설명하는 말
    combo_label = Label(tab3_frame2, text="원하는 조합을 골라보세요!!\n     ↓    ")
    combo_label.grid(row=3, column=7)

    Button(tab3_frame2, text="저장", width=10, bg="skyblue", command=shot).grid(row=9, column=7)
...
def OUT1():
    try:
        ...
        messagebox.showinfo("성공", "선택한 과목을 삭제했습니다")
    except:
        if len(selected_groups[0]) == 0:
            messagebox.showinfo("오류", "삭제할 과목이 없습니다")
        else:
            messagebox.showinfo("오류", "삭제할 과목을 선택해주세요")
...
Radiobutton(groups_frame, text="그룹-1", value=0, variable=variety1).pack()
Radiobutton(groups_frame, text="그룹-2", value=1, variable=variety1).pack()
Radiobutton(groups_frame, text="그룹-3", value=2, variable=variety1).pack()
Radiobutton(groups_frame, text="그룹-4", value=3, variable=variety1).pack()
Radiobutton(groups_frame, text="그룹-5", value=4, variable=variety1).pack()
Radiobutton(groups_frame, text="그룹-6", value=5, variable=variety1).pack()
Radiobutton(groups_frame, text="그룹-7", value=6, variable=variety1).pack()
Radiobutton(groups_frame, text="그룹-8", value=7, variable=variety1).pack()
Radiobutton(groups_frame, text="그룹-9", value=8, variable=variety1).pack()
Radiobutton(groups_frame, text="그룹-10", value=9, variable=variety1).pack()
...
########   tab 2 (그룹보기)   ########
#####################################
group_list = ["그룹-1", "그룹-2", "그룹-3", "그룹-4", "그룹-5", "그룹-6", "그룹-7", "그룹-8", "그룹-9", "그룹-10"]
...
Button(tab2, text="그룹-1 제거", width=10, bg='skyblue', command=OUT1).grid(row=0, column=2, rowspan=1)
Button(tab2, text="그룹-2 제거", width=10, bg='skyblue', command=OUT2).grid(row=1, column=2, rowspan=1)
Button(tab2, text="그룹-3 제거", width=10, bg='skyblue', command=OUT3).grid(row=2, column=2, rowspan=1)
Button(tab2, text="그룹-4 제거", width=10, bg='skyblue', command=OUT4).grid(row=3, column=2, rowspan=1)
Button(tab2, text="그룹-5 제거", width=10, bg='skyblue', command=OUT5).grid(row=4, column=2, rowspan=1)
Button(tab2, text="그룹-6 제거", width=10, bg='skyblue', command=OUT6,).grid(row=5, column=2, rowspan=1)
Button(tab2, text="그룹-7 제거", width=10, bg='skyblue', command=OUT7).grid(row=6, column=2, rowspan=1)
Button(tab2, text="그룹-8 제거", width=10, bg='skyblue', command=OUT8).grid(row=7, column=2, rowspan=1)
Button(tab2, text="그룹-9 제거", width=10, bg='skyblue', command=OUT9).grid(row=8, column=2, rowspan=1)
Button(tab2, text="그룹-10 제거", width=10, bg='skyblue', command=OUT10).grid(row=9, column=2)
Button(tab2, text="생성", width=10, bg='pink', command=combinating).grid(row=10, column=2)
...
def show_shot_img():
    ...
    try:
        ...
        messagebox.showinfo("오류", "사진파일을 선택하세요")
        return
...
# 기타 나머지
Button(tab1, text="나가기", width=10, bg="slateblue2", fg="white", command=exit_window).grid(row=17, column=0)
Label(tab2).grid(row=11, column=2) # 공백
Button(tab2, text="나가기", width=10, bg="slateblue2", fg="white", command=exit_window).grid(row=12, column=2)
Button(tab3, text="나가기", width=10, bg="slateblue2", fg="white", command=exit_window).grid(row=2, column=0, sticky='w')
Button(tab4, text="삭제", width=10, bg="red", fg="white", command=file_delete).grid(row=2, column=1)
Button(tab4, text="나가기", width=10, bg="slateblue2", fg="white", command=exit_window).grid(row=3, column=1)
window.mainloop()