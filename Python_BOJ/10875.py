...
class Line:
    ...
    def isCrossingOtherLine(self, other_line):
        ...
        if p1p2 == 0 and p3p4 == 0: # 일직선 상
            result = p3 <= p2 and p1 <= p4 # 겹침여부
            if result: return [True, p3[0] - p1[0]] if p1[1] == p2[1] else [True, p3[1] - p1[1]]
        else: # 수직으로 교차
...
for dt, next_direction in [input().split() for _ in range(N)]:
    ...
    if len(lines) >= 3:
        closest_line_distance = 3 * L
        for old_line in lines[:-2]: # 최근 두개는 충돌가능성 없음.
            result = new_line.isCrossingOrOutOfBoundary(old_line)
            if result[0]:
                isSnakeDead = True
                if result[1] < closest_line_distance: closest_line_distance = result[1]
        if closest_line_distance < 3 * L: new_line.size = closest_line_distance
